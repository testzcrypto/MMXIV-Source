...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("mmxiv"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    {
        uri.replace(0, std::string("mmxiv://").length(), "mmxiv:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MMXIV.lnk";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MMXIV-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  mmxiv-qt [" + tr("command-line options") + "]                     " + "\n";
